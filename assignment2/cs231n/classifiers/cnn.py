...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        C, H, W = input_dim
        HP, HW = 1 + (H - 2)//2, 1 + (W - 2)//2   # Max-pooling

        self.params['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
        self.params['b1'] = np.zeros(num_filters)
        self.params['W2'] = weight_scale * np.random.randn(num_filters * HP * HW, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        dout, grads['W3'], grads['b3'] = affine_backward(dsoftmax, cache_scores)
