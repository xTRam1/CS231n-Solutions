...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, grads['W2'], grads['b2'] = affine_backward(grad_softmax, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        layer_inputs = np.hstack((input_dim, hidden_dims, num_classes))
        for i in range(self.num_layers):
            self.params['W'+str(i+1)] = weight_scale * np.random.randn(layer_inputs[i], layer_inputs[i+1])
            self.params['b'+str(i+1)] = np.zeros(layer_inputs[i+1])
        
        if self.normalization != None:
            for i in range(self.num_layers-1):
                self.params['gamma'+str(i+1)] = np.ones(layer_inputs[i+1])
                self.params['beta'+str(i+1)] = np.zeros(layer_inputs[i+1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                scores, last_cache = affine_forward(x, w, b)
                caches.append(last_cache)
            else:
                gamma, beta, bn_param = -1, -1, None
                if self.normalization != None:
                    gamma = self.params['gamma'+str(i+1)]
                    beta = self.params['beta'+str(i+1)]
                    bn_param = self.bn_params[i]
                hidden, cache = affine_norm_relu_forward(x, w, b, gamma, beta, bn_param, self.normalization, self.use_dropout, self.dropout_param)
                x = hidden
                caches.append(cache)
        ...
        for i in range(self.num_layers - 1):
            dx, dw, db, dgamma, dbeta = affine_norm_relu_backward(dx, caches[self.num_layers - (i+2)], self.normalization, self.use_dropout)
            if self.normalization != None:
...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param, norm, dropout, dropout_param):
    out, fc_cache = affine_forward(x, w, b)
    drop_cache, batch_cache = None, None
    if norm == 'batchnorm': 
